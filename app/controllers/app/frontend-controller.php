...
/**
 * Public-facing controller for native-app page responses.
 *
 * The `app.boot` boot-config endpoint was RETIRED — the app config is now
 * baked into the binary at build time (the host app fetches a *customer's*
 * config only via `app.preview_config` when previewing). What remains here are
 * the always-live `wp_head` injections (app CSS, ads blocker, analytics,
 * runtime app-settings) and the first-install / booted bookkeeping hooks.
 */
class Frontend_Controller extends Base_Controller
{
	...
	{
		// `app.boot` retired — the app config is now baked into the binary at
		// build time for BOTH customer and host builds (the host app fetches a
		// *customer's* config only via `app.preview_config` when previewing).
		// No runtime boot-config endpoint.
