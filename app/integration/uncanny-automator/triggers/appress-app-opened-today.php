...
/**
 * Trigger: user opens the Appress app for the first time today.
 * Single pill: app selector.
 */
class APPRESS_APP_OPENED_TODAY extends \Uncanny_Automator\Recipe\Trigger {

	protected function setup_trigger() {
		$this->set_integration( 'APPRESS' );
		$this->set_trigger_code( 'APPRESS_APP_OPENED_TODAY' );
		$this->set_trigger_meta( 'APPRESS_APP_OPENED_TODAY_APP' );
		$this->set_is_login_required( true );
		$this->set_sentence(
			sprintf(
				/* translators: %1$s: app selector */
				esc_attr_x( 'A user opens {{an Appress app:%1$s}} for the first time today', 'Appress', 'appress' ),
				$this->get_trigger_meta()
			)
		);
		$this->set_readable_sentence( esc_attr_x( 'A user opens {{an Appress app}} for the first time today', 'Appress', 'appress' ) );

		$this->add_action( 'appress/automator/opened_today', 10, 2 );
	}
