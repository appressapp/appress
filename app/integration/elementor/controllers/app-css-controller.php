...
namespace Appress\Integration\Elementor\Controllers;
...
/**
 * Elementor-specific CSS rules appended to the Appress mobile-app CSS
...
class App_Css_Controller extends \Appress\Controllers\Base_Controller {
	...
	protected function hooks() {
		// Bail if Elementor isn't loaded — no point shipping
		// Elementor-specific CSS to sites that don't render Elementor.
	...
	private function rules_for_platform( $platform ) {
		switch ( $platform ) {
			case 'all':
				return <<<'CSS'
/* Elementor — cross-platform tweaks for Appress mobile app */
...
CSS;
			...
			case 'android':
				return <<<'CSS'
/* Elementor — Android-specific tweaks */
CSS;

			case 'ios':
				return <<<'CSS'
/* Elementor — iOS-specific tweaks */
CSS;
		}
		return '';
	}
